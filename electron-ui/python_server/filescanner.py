...
def foundMediaFile(path):
    ...
    added = int(time.time())
    fff = 'pending'
    cur.execute("insert or ignore into library (path,basename,size,modified,added,fff) values (?,?,?,?,?,?)",
                [realpath, basename, size, modified, added, fff])
    con.commit()
    print("inserted")
...
def listdir(root, depth, parentCounts):
    ...
    for dir in subdirs:
        ...
        if depth + 1 <= max_search_depth:
            ...
            print(dir)
            print("MAXDEPTH not going any deeper")
